...
typedef void (^MLAudioRecorderReceiveStartBlock)();
typedef void (^MLAudioRecorderReceiveContinueBlock)();
typedef void (^MLAudioRecorderReceivePauseBlock)();
typedef void (^MLAudioRecorderReceiveBeginInterruptionBlock)();
typedef void (^MLAudioRecorderReceiveEndInterruptionBlock)();
/**
 *  错误标识
 */
...
typedef enum {
    OutputQualityLow = 7,
    OutputQualityMiddle = 5,
    OutputQualityHight = 2
}OutputQuality;
 ...
 */
@property (atomic, assign) unsigned long sampleRate;
@property (atomic, assign) double bufferDurationSeconds;
/**
 *  通道个数 1:单通道  2:双通道(立体声)
 */
@property (atomic, assign) int numChannels;
@property (atomic, assign) int bitsPerChannel;
/**
 *  输出码率kbps,default:32kbps
 */
@property (atomic, assign) int outputBitRate;
/**
 *  输出的转码mp3后的音质
 */
@property (atomic, assign) OutputQuality outputQuality;
/**
 *  输出的转码mp3后的最大时间
 */
@property (atomic, assign) NSTimeInterval outputMaxTimeSeconds;
/**
 *  输出的转码mp3后的最大文件大小(Mb)
 */
@property (atomic, assign) float outputMaxFileSize;
/**
 *  输出的转码mp3后的文件路径
 */
@property (atomic, copy) NSString *outputFilePath;
...
@property (nonatomic, copy) MLAudioRecorderReceiveContinueBlock receiveContinueBlock;
@property (nonatomic, copy) MLAudioRecorderReceivePauseBlock receivePauseBlock;
@property (nonatomic, copy) MLAudioRecorderReceiveBeginInterruptionBlock beginInterruptionBlock;
@property (nonatomic, copy) MLAudioRecorderReceiveEndInterruptionBlock endInterruptionBlock;
@property (nonatomic, copy) MLAudioRecorderReceiveStartBlock receiveStartBlock;
