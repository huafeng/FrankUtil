 ...
 */
#define kDefaultBufferDurationSeconds 0.15
/**
...
@property (nonatomic) dispatch_queue_t writeFileQueue;
@property (nonatomic) dispatch_semaphore_t semError; //一个信号量，用来保证队列中写文件错误事件处理只调用一次
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.writeFileQueue = dispatch_queue_create("com.yiyu.LabaAssignment.writeFileQueue", NULL);
        
        self.sampleRate = kDefaultSampleRate;
        self.numChannels = 1;
        self.bitsPerChannel = 16;
        self.bufferDurationSeconds = kDefaultBufferDurationSeconds;
        self.outputBitRate = 32;
        self.outputQuality = OutputQualityMiddle;
        //ios 6 or later
...
- (void)startRecording
{
    ...
    if (self.isPaused) {
        self.isPaused = NO;
        //启用audio session
        BOOL ret = [[AVAudioSession sharedInstance] setActive:YES error:nil];
        if (!ret)
        {
        ...
        if (self.receiveContinueBlock){
            self.receiveContinueBlock();
        }
        return;
    }
    ...
    self.isRecording = YES;
    if (self.receiveStartBlock) {
        self.receiveStartBlock();
    }
}
...
- (void)setupAudioFormat:(UInt32) inFormatID SampleRate:(int)sampeleRate
{
    ...
    _recordFormat.mChannelsPerFrame = self.numChannels;//(UInt32)[[AVAudioSession sharedInstance] inputNumberOfChannels];
	...
	if (inFormatID == kAudioFormatLinearPCM){
        ...
        //每个通道里，一帧采集的bit数目
		_recordFormat.mBitsPerChannel = self.bitsPerChannel;
        //结果分析: 8bit为1byte，即为1个通道里1帧需要采集2byte数据，再*通道数，即为所有通道采集的byte数目。
        //所以这里结果赋值给每帧需要采集的byte数目，然后这里的packet也等于一帧的数据。
        //至于为什么要这样。。。不知道。。。
		_recordFormat.mBytesPerPacket = _recordFormat.mBytesPerFrame = (_recordFormat.mBitsPerChannel / 8) * _recordFormat.mChannelsPerFrame;
        //subdivides the raw bytes of the packet into some number of frames.This is an issue only for compressed formats; uncompressed formats always set this to 1
...
- (void)beginInterruptionAction {
    ...
    //        [this stopRecording];
    [[AVAudioSession sharedInstance] setActive:NO error:nil];
    if (self.beginInterruptionBlock) {
        self.beginInterruptionBlock();
    }
}
...
- (void)endInterruptionAction {
    ...
    NSLog(@"从打断中恢复,继续录音");
    //        [this startRecording];
    if (self.endInterruptionBlock) {
        self.endInterruptionBlock();
    }
}
...
- (void)addRouteChangedListener {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 6.0) {
        ...
        AudioSessionAddPropertyListener(kAudioSessionProperty_AudioRouteChange,
                                         audioRecordRouteChangeListenerCallback,
                                         (__bridge void *)(self));
    }
}
...
- (void)removeRouteListener {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 6.0) {
        ...
        AudioSessionRemovePropertyListenerWithUserData(kAudioSessionProperty_AudioRouteChange, audioRecordRouteChangeListenerCallback, (__bridge void *)(self));
    }
}
...
void audioRecordRouteChangeListenerCallback(void *inUserData,
    ...
    if (routeChangeReason == kAudioSessionRouteChangeReason_OldDeviceUnavailable) {
        MLAudioRecorder* this = (__bridge MLAudioRecorder *)inUserData;
...
- (void)dealloc
{
    ...
    _receiveContinueBlock = nil;
    _receiveErrorBlock = nil;
    _receiveStoppedBlock = nil;
    _receivePauseBlock = nil;
    _beginInterruptionBlock = nil;
    _endInterruptionBlock = nil;
    _receiveStartBlock = nil;
